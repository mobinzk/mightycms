...
class Markdown {
	...
	#
		...
		$text = $parser->parseImages($text);
		$text = $parser->transform($text); 
		$text = $parser->removeImageParagraphs($text);
	...
	public function parseLinks($text){
		...
		if (count($matches[1]) > 0){
			foreach ($matches[1] as $key => $val){
				...
				// Force external urls to open in new window
				if (preg_match('/(http:\/\/|https:\/\/)/si', $val)){
					$content = str_replace('href="'.$val.'"', 'href="'.$val.'" target="_blank"', $content);
				}

				// Force files to open in new window
				if (preg_match('/.*\.(pdf|doc|docx)/si', $val)){
					$content = str_replace('href="'.$val.'"', 'href="'.$val.'" target="_blank"', $content);
				}
	...
	public function removeImageParagraphs($text){
		$text = preg_replace('/<p><img(.*)class="(.*?)"(.*?)\/><\/p>/', "<img$1class=\"$2\"$3/>", $text);
		return $text;
	}

	public function parseImages($text){
		$text = preg_replace('/!\[(.*?)\]\((.*?)\)\((.*?)\)/', "<img src=\"$2\" alt=\"$1\" style=\"float:$3\" class=\"$3\" />", $text);
		return $text;
	}
